...
- (void)setThumbnailDataFromImage:(UIImage *)image
{
    ...
    // Figure out a scaling ratio to make sure we maintain aspect ratio
    float ratio = MAX(newRect.size.width / origImageSize.width, newRect.size.height / origImageSize.height);
    
    // Create a transparent bitmap context w/scaling factor equal to screen
    UIGraphicsBeginImageContextWithOptions(newRect.size, NO, 0.0);
    
    
    // Crate a path that is a rounded rectangle
    ...
    // Get the PNG rep of the image and set it as archivable data
    NSData *data = UIImagePNGRepresentation(smallImage);
    self.thumbnailData = data;
                    
    // Clean up image context resouces
